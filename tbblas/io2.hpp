...
template<class T, unsigned dim, bool device>
void print(const tbblas::proxy<tbblas::tensor<T, dim, device> >& proxy, int precision = 5,
    std::ostream& out = std::cout)
{
  tbblas::tensor<T, dim, device> tensor = proxy;
...
typename boost::enable_if<tbblas::is_expression<Expression>,
  std::ostream
>::type&
operator<<(std::ostream& os, const Expression& expression) {
  tbblas::tensor<typename Expression::value_t, Expression::dimCount, false> t = expression;
  print(t, 5, os);
  return os;
}
