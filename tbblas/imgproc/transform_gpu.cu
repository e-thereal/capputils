...
__global__ void transform4DKernel(float *d_result, tensor<float, 4>::dim_t dimension, fmatrix4 transMatrix, uint k, uint c) {
  uint i = (blockDim.x * blockIdx.x + threadIdx.x);
  uint j = (blockDim.y * blockIdx.y + threadIdx.y);

  if (i >= dimension[0] || j >= dimension[1] || k >= dimension[2] || c >= dimension[3])
    return;
...
void transform(tensor<float, 4, true>& input, const fmatrix4& matrix, tensor<float, 4, true>& output) {
  ...
  const cudaExtent volumeSize = make_cudaExtent(input.size()[0], input.size()[1], input.size()[2] * input.size()[3]);
  cudaChannelFormatDesc texDesc = cudaCreateChannelDesc<float>();
  cudaMalloc3DArray(&d_cudaArray, &texDesc, volumeSize);
