...
#ifndef TBBLAS_DEEPLEARN_SERIALIZE_CONV_DBN_HPP_
#define TBBLAS_DEEPLEARN_SERIALIZE_CONV_DBN_HPP_
...
void serialize(const tbblas::deeplearn::conv_dbn_model<T, dims>& model, std::ostream& out) {
  unsigned count = 0;
...
template<class T, unsigned dims>
void serialize(const tbblas::deeplearn::conv_dbn_model<T, dims>& model, const std::string& filename) {
  std::ofstream out(filename.c_str(), std::ios_base::binary);
  serialize(model, out);
}

template<class T, unsigned dims>
void deserialize(std::istream& in, tbblas::deeplearn::conv_dbn_model<T, dims>& model) {
...
template<class T, unsigned dims>
void deserialize(const std::string& filename, tbblas::deeplearn::conv_dbn_model<T, dims>& model) {
...
#endif /* TBBLAS_DEEPLEARN_SERIALIZE_CONV_DBN_HPP_ */
