...
#ifndef TBBLAS_DEEPLEARN_OPT_VSGD_FD_HPP_
#define TBBLAS_DEEPLEARN_OPT_VSGD_FD_HPP_

/*
 * This optimizer implements the "No more pesky learning rates" update. But instead of using bbprop to estimate
...
private:
  const vsgd_fd* _parent;
  value_t _epsilon, _c;
  v_vector_t _delta, _old_grad, _g, _g2, _h, _tau;
...
public:
  ...
  vsgd_fd(const vsgd_fd* parent) : _parent(parent) { }

  template<class Expression>
  void update_delta(const Expression& gradient, int index) {

    if (index >= _delta.size() || index >= _old_grad.size() || index >= _g.size() || index >= _g2.size() || index >= _h.size() || index >= _tau.size() || index >= _current_iteration.size()) {
    ...
    if (!_delta[index] || !_old_grad[index] || !_g[index] || !_g2[index] || !_h[index] || !_tau[index]) {
