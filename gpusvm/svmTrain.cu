...
#define SHOW_ADDR(a) std::cout << #a << ": " << ((char*)&kernelCache - (char*)&kernelCache. a) << std::endl;

void performTraining(float* data, int nPoints, int nDimension, float* labels, float** p_alpha, Kernel_params* kp, float cost, int maxIterationCount, SelectionHeuristic heuristicMethod, float epsilon, float tolerance, float* transposedData) {
  ...
  *p_alpha = alpha;
  ...
  int blockWidth = intDivideRoundUp(nPoints, GPUSVM_BLOCKSIZE);
  ...
  dim3 reduceThreads(GPUSVM_BLOCKSIZE);

  
  printf("Starting iterations\n");
	
  for (iteration = 1; iteration < maxIterationCount; iteration++) {
