#ifndef GPUSVM_FIRSTORDER_H
#define GPUSVM_FIRSTORDER_H
...
template<bool iLowCompute, bool iHighCompute, class Kernel>
  ...
  extern __shared__ float xIHigh[];
	float* xILow;
	__shared__ int tempLocalIndices[GPUSVM_BLOCKSIZE];
	__shared__ float tempLocalFs[GPUSVM_BLOCKSIZE];
  ...
  if (iLowCompute) {
	...
	int globalIndex = GPUSVM_IMUL(blockDim.x, blockIdx.x) + threadIdx.x;
  ...
  int reduceFlag;
    ...
    alpha = devAlpha[globalIndex];
		...
		if (alpha > epsilon) {
			if (alpha < cEpsilon) {
				reduceFlag = GPUSVM_REDUCE0 | GPUSVM_REDUCE1; //Unbound support vector (I0)
			} else {
				if (label > 0) {
					reduceFlag = GPUSVM_REDUCE0; //Bound positive support vector (I3)
				} else {
					reduceFlag = GPUSVM_REDUCE1; //Bound negative support vector (I2)
				}
			}
		} else {
			if (label > 0) {
				reduceFlag = GPUSVM_REDUCE1; //Positive nonsupport vector (I1)
			} else {
				reduceFlag = GPUSVM_REDUCE0; //Negative nonsupport vector (I4)
			}
		}
	} else {
		reduceFlag = GPUSVM_NOREDUCE;
	}
  ...
  if ((reduceFlag & GPUSVM_REDUCE0) == 0) {
  ...
  if ((reduceFlag & GPUSVM_REDUCE1) == 0) {
    tempLocalFs[threadIdx.x] = FLT_MAX; //Ignore me
  } else {
    tempLocalFs[threadIdx.x] = f;
    tempLocalIndices[threadIdx.x] = globalIndex;
...
__global__ void firstOrderPhaseTwo(float* devData, int devDataPitchInFloats, float* devTransposedData, int devTransposedDataPitchInFloats, float* devLabels, float* devKernelDiag, float* devAlpha, void* devResult, float cost, int nDimension, float parameterA, float parameterB, float parameterC, int* devLocalIndicesRL, int* devLocalIndicesRH, float* devLocalFsRL, float* devLocalFsRH, int inputSize) {
  __shared__ int tempIndices[GPUSVM_BLOCKSIZE];
  __shared__ float tempFs[GPUSVM_BLOCKSIZE];
  ...
  if (inputSize > GPUSVM_BLOCKSIZE) {
    for (int i = threadIdx.x + GPUSVM_BLOCKSIZE; i < inputSize; i += blockDim.x) {
